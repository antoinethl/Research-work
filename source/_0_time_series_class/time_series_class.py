...
class TimeSeries():
    ...
    def discretize(self, threshold=100):
        ...
        self.ts_dis = clustering(self.df, self.conf["nb_clust"])
    ...
    def plot_series(self, t1=0, t2=100, t1p=None, t2p=None):
        ...
        plot_discretized(self.df["active_power"], self.ts_dis, t1=t1, t2=t2, t1p=t1p, t2p=t2p)
    ...
    def compute_error(self):
        ...
        self.predict['error'] = self.error.apply(lambda r: tuple(r), axis=1).apply(np.array)
    ...
    def compute_weights_init(self, size_max=30, learning_rate=0.1):
        ...
        self.error[f"mae final"] = self.predict[["final", "test"]].apply(lambda x: mae(x), axis=1)
    ...
    def plot_predictions(self, names=None, min_=101, max_=1000):
